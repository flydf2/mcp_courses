...
func main() {
	...
	serverMode := utils.GetEnvOrDefault("SERVER_MODE", "stdio")

	// 加载配置
	apiBaseURL := utils.GetEnvOrDefault("API_BASE_URL", "http://localhost:8084")
	apiAuthToken := utils.GetEnvOrDefault("API_AUTH_TOKEN", "")
	servicePort := utils.GetEnvOrDefault("SERVER_MODE", "6081")

	// 初始化外部服务客户端
	edtClient := client.NewEDTClient(apiBaseURL, apiAuthToken)
